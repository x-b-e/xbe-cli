#!/bin/bash
#
# XBE CLI Integration Tests: Trailer Credentials
#
# Tests CRUD operations for the trailer_credentials resource.
# Trailer credentials track credentials (registrations, permits) for trailers.
#
# NOTE: Each create test uses a unique classification because credentials
# for the same trailer+classification cannot have overlapping date ranges.
#
...
describe "Resource: trailer_credentials"

# ============================================================================
# Prerequisites - Create broker, trucker, trailer, and classification
# ============================================================================

test_name "Create prerequisite broker for trailer credentials tests"
BROKER_NAME=$(unique_name "TrCTestBroker")
...
TRUCKER_NAME=$(unique_name "TrCTestTrucker")
...
TRAILER_NUMBER=$(unique_name "TrCTrailer")
...
CLASS_NAME=$(unique_name "TrCTestClass")
...
test_name "Create trailer credential with required fields"

xbe_json do trailer-credentials create \
...
if [[ $status -eq 0 ]]; then
    ...
    if [[ -n "$CREATED_CREDENTIAL_ID" && "$CREATED_CREDENTIAL_ID" != "null" ]]; then
        register_cleanup "trailer-credentials" "$CREATED_CREDENTIAL_ID"
        pass
    else
        fail "Created trailer credential but no ID returned"
    fi
else
    fail "Failed to create trailer credential"
fi

# Only continue if we successfully created a credential
if [[ -z "$CREATED_CREDENTIAL_ID" || "$CREATED_CREDENTIAL_ID" == "null" ]]; then
    echo "Cannot continue without a valid trailer credential ID"
    run_tests
fi

test_name "Create trailer credential with issued-on"
# Create a unique classification to avoid overlap
CLASS_NAME2=$(unique_name "TrCTestClass2")
xbe_json do tractor-trailer-credential-classifications create \
    --name "$CLASS_NAME2" \
    --organization-type "truckers" \
    --organization-id "$CREATED_TRUCKER_ID"
if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -eq 0 ]]; then
        id=$(json_get ".id")
        register_cleanup "trailer-credentials" "$id"
        pass
    else
        fail "Failed to create trailer credential with issued-on"
    fi
else
    fail "Failed to create classification for issued-on test"
fi

test_name "Create trailer credential with expires-on"
# Create a unique classification to avoid overlap
CLASS_NAME3=$(unique_name "TrCTestClass3")
xbe_json do tractor-trailer-credential-classifications create \
    --name "$CLASS_NAME3" \
    --organization-type "truckers" \
    --organization-id "$CREATED_TRUCKER_ID"
if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -eq 0 ]]; then
        ...
        fail "Failed to create trailer credential with expires-on"
    fi
else
    fail "Failed to create classification for expires-on test"
fi

test_name "Create trailer credential with both dates"
# Create a unique classification to avoid overlap
CLASS_NAME4=$(unique_name "TrCTestClass4")
xbe_json do tractor-trailer-credential-classifications create \
    --name "$CLASS_NAME4" \
    --organization-type "truckers" \
    --organization-id "$CREATED_TRUCKER_ID"
if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -eq 0 ]]; then
        ...
        fail "Failed to create trailer credential with both dates"
    fi
else
...
test_name "Update trailer credential issued-on"
xbe_json do trailer-credentials update "$CREATED_CREDENTIAL_ID" --issued-on "2024-02-01"
assert_success

test_name "Update trailer credential expires-on"
xbe_json do trailer-credentials update "$CREATED_CREDENTIAL_ID" --expires-on "2026-02-01"
assert_success
...
test_name "List trailer credentials"
xbe_json view trailer-credentials list --limit 5
assert_success

test_name "List trailer credentials returns array"
xbe_json view trailer-credentials list --limit 5
if [[ $status -eq 0 ]]; then
    assert_json_is_array
else
    fail "Failed to list trailer credentials"
fi
...
test_name "List trailer credentials with --trailer filter"
xbe_json view trailer-credentials list --trailer "$CREATED_TRAILER_ID" --limit 10
assert_success
...
test_name "List trailer credentials with --limit"
xbe_json view trailer-credentials list --limit 3
assert_success

test_name "List trailer credentials with --offset"
xbe_json view trailer-credentials list --limit 3 --offset 3
assert_success
...
test_name "Delete trailer credential requires --confirm flag"
xbe_run do trailer-credentials delete "$CREATED_CREDENTIAL_ID"
assert_failure

test_name "Delete trailer credential with --confirm"
# Create a credential specifically for deletion (using unique classification)
CLASS_NAME_DEL=$(unique_name "TrCTestClassDel")
xbe_json do tractor-trailer-credential-classifications create \
    --name "$CLASS_NAME_DEL" \
    --organization-type "truckers" \
    --organization-id "$CREATED_TRUCKER_ID"
if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -eq 0 ]]; then
        DEL_ID=$(json_get ".id")
        xbe_run do trailer-credentials delete "$DEL_ID" --confirm
        assert_success
    else
        skip "Could not create trailer credential for deletion test"
    fi
else
...
test_name "Create trailer credential without trailer fails"
xbe_json do trailer-credentials create \
    --tractor-trailer-credential-classification "$CREATED_CLASSIFICATION_ID"
assert_failure

test_name "Create trailer credential without classification fails"
xbe_json do trailer-credentials create \
    --trailer "$CREATED_TRAILER_ID"
assert_failure

test_name "Update without any fields fails"
xbe_json do trailer-credentials update "$CREATED_CREDENTIAL_ID"
assert_failure
