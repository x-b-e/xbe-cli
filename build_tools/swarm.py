...
def run_recover(config: str) -> None:
    recover_path = Path(__file__).with_name("recover.py")
    cmd = [sys.executable, str(recover_path), "--config", config]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a swarm of Cartographer workers")
