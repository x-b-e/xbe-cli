...
type equipmentRequirementsShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type equipmentRequirementDetails struct {
	ID                                         string   `json:"id"`
	...
	InboundMovementRequirementID               string   `json:"inbound_movement_requirement_id,omitempty"`
	OutboundMovementRequirementID              string   `json:"outbound_movement_requirement_id,omitempty"`
...
func newEquipmentRequirementsShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show equipment requirement details",
		Long: `Show the full details of an equipment requirement.
...
Output Fields:
  ...
  Time Sheet
  Inbound/Outbound Movement Requirements
  Start At / End At
...
Arguments:
  <id>    The equipment requirement ID (required). You can find IDs using the list command.`,
		Example: `  # Show an equipment requirement
  xbe view equipment-requirements show 123

  # Show as JSON
  xbe view equipment-requirements show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runEquipmentRequirementsShow,
	}
	initEquipmentRequirementsShowFlags(cmd)
	return cmd
}

func init() {
	equipmentRequirementsCmd.AddCommand(newEquipmentRequirementsShowCmd())
}

func initEquipmentRequirementsShowFlags(cmd *cobra.Command) {
...
func runEquipmentRequirementsShow(cmd *cobra.Command, args []string) error {
	opts, err := parseEquipmentRequirementsShowOptions(cmd)
	if err != nil {
	...
	details := buildEquipmentRequirementDetails(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), details)
	}

	return renderEquipmentRequirementDetails(cmd, details)
}

func parseEquipmentRequirementsShowOptions(cmd *cobra.Command) (equipmentRequirementsShowOptions, error) {
	jsonOut, _ := cmd.Flags().GetBool("json")
	noAuth, _ := cmd.Flags().GetBool("no-auth")
	baseURL, _ := cmd.Flags().GetString("base-url")
	token, _ := cmd.Flags().GetString("token")

	return equipmentRequirementsShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildEquipmentRequirementDetails(resp jsonAPISingleResponse) equipmentRequirementDetails {
	resource := resp.Data
	attrs := resource.Attributes
	details := equipmentRequirementDetails{
		ID:                           resource.ID,
	...
	if rel, ok := resource.Relationships["inbound-movement-requirement"]; ok && rel.Data != nil {
		details.InboundMovementRequirementID = rel.Data.ID
	}
	if rel, ok := resource.Relationships["outbound-movement-requirement"]; ok && rel.Data != nil {
		details.OutboundMovementRequirementID = rel.Data.ID
	}
...
func renderEquipmentRequirementDetails(cmd *cobra.Command, details equipmentRequirementDetails) error {
	out := cmd.OutOrStdout()
	...
	if details.InboundMovementRequirementID != "" {
		fmt.Fprintf(out, "Inbound Movement Requirement: %s\n", details.InboundMovementRequirementID)
	}
	if details.OutboundMovementRequirementID != "" {
		fmt.Fprintf(out, "Outbound Movement Requirement: %s\n", details.OutboundMovementRequirementID)
	}
	if details.StartAt != "" {
