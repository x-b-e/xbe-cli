...
type liabilityIncidentsListOptions struct {
	BaseURL                       string
	Token                         string
	JSON                          bool
	NoAuth                        bool
...
type liabilityIncidentRow struct {
	ID               string `json:"id"`
...
func newLiabilityIncidentsListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List liability incidents",
		Long: `List liability incidents with filters and pagination.
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
  ...
  xbe view liability-incidents list --status open --kind damage

  # Filter by net impact
  xbe view liability-incidents list --net-impact-minutes-min 15 --net-impact-dollars-min 100

  # Filter by subject
  xbe view liability-incidents list --subject \"Customer|123\"

  # Output as JSON
  xbe view liability-incidents list --json`,
		RunE: runLiabilityIncidentsList,
	}
	initLiabilityIncidentsListFlags(cmd)
	return cmd
}

func init() {
	liabilityIncidentsCmd.AddCommand(newLiabilityIncidentsListCmd())
}

func initLiabilityIncidentsListFlags(cmd *cobra.Command) {
...
func runLiabilityIncidentsList(cmd *cobra.Command, _ []string) error {
	opts, err := parseLiabilityIncidentsListOptions(cmd)
	if err != nil {
	...
	query := url.Values{}
	query.Set("fields[liability-incidents]", "status,kind,severity,start-at,net-impact-minutes,net-impact-dollars,is-down-time,subject")
	...
	body, _, err := client.Get(cmd.Context(), "/v1/liability-incidents", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildLiabilityIncidentRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderLiabilityIncidentsTable(cmd, rows)
}

func parseLiabilityIncidentsListOptions(cmd *cobra.Command) (liabilityIncidentsListOptions, error) {
	...
	return liabilityIncidentsListOptions{
		BaseURL:                       baseURL,
		Token:                         token,
		JSON:                          jsonOut,
		NoAuth:                        noAuth,
...
func buildLiabilityIncidentRows(resp jsonAPIResponse) []liabilityIncidentRow {
	rows := make([]liabilityIncidentRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		row := buildLiabilityIncidentRow(resource)
		rows = append(rows, row)
	}
	return rows
}

func buildLiabilityIncidentRow(resource jsonAPIResource) liabilityIncidentRow {
	row := liabilityIncidentRow{
		ID:               resource.ID,
...
func buildLiabilityIncidentRowFromSingle(resp jsonAPISingleResponse) liabilityIncidentRow {
	return buildLiabilityIncidentRow(resp.Data)
}

func renderLiabilityIncidentsTable(cmd *cobra.Command, rows []liabilityIncidentRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No liability incidents found.")
		return nil
	}

	writer := tabwriter.NewWriter(cmd.OutOrStdout(), 2, 4, 2, ' ', 0)
	fmt.Fprintln(writer, "ID\tSTATUS\tKIND\tSEVERITY\tNET MIN\tNET $\tDOWN TIME\tSTART AT\tSUBJECT")
