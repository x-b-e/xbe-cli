...
type efficiencyIncidentsShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type efficiencyIncidentDetails struct {
	ID                   string   `json:"id"`
...
func newEfficiencyIncidentsShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show efficiency incident details",
		Long: `Show the full details of an efficiency incident.
...
Arguments:
  <id>    The efficiency incident ID (required). Use the list command to find IDs.

Global flags (see xbe --help): --json, --base-url, --token, --no-auth`,
		Example: `  # Show an efficiency incident
  xbe view efficiency-incidents show 123

  # JSON output
  xbe view efficiency-incidents show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runEfficiencyIncidentsShow,
	}
	initEfficiencyIncidentsShowFlags(cmd)
	return cmd
}

func init() {
	efficiencyIncidentsCmd.AddCommand(newEfficiencyIncidentsShowCmd())
}

func initEfficiencyIncidentsShowFlags(cmd *cobra.Command) {
...
func runEfficiencyIncidentsShow(cmd *cobra.Command, args []string) error {
	opts, err := parseEfficiencyIncidentsShowOptions(cmd)
	if err != nil {
	...
	if id == "" {
		return fmt.Errorf("efficiency incident id is required")
	}
	...
	body, _, err := client.Get(cmd.Context(), "/v1/efficiency-incidents/"+id, query)
	if err != nil {
		if len(body) > 0 {
...
func parseEfficiencyIncidentsShowOptions(cmd *cobra.Command) (efficiencyIncidentsShowOptions, error) {
	jsonOut, _ := cmd.Flags().GetBool("json")
	noAuth, _ := cmd.Flags().GetBool("no-auth")
	baseURL, _ := cmd.Flags().GetString("base-url")
	token, _ := cmd.Flags().GetString("token")

	return efficiencyIncidentsShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildEfficiencyIncidentDetails(resp jsonAPISingleResponse) efficiencyIncidentDetails {
	resource := resp.Data
	attrs := resource.Attributes
	details := efficiencyIncidentDetails{
		ID:                   resource.ID,
		Type:                 resource.Type,
...
func renderEfficiencyIncidentDetails(cmd *cobra.Command, details efficiencyIncidentDetails) error {
	out := cmd.OutOrStdout()
