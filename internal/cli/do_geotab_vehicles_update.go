...
type doGeotabVehiclesUpdateOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	ID      string
...
func newDoGeotabVehiclesUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update geotab vehicle assignments",
		Long: `Update geotab vehicle assignments.

Common flags:
  --trailer    Update the trailer assignment
  --tractor    Update the tractor assignment`,
		Example: `  # Update trailer assignment
  xbe do geotab-vehicles update 123 --trailer 456

  # Update tractor assignment
  xbe do geotab-vehicles update 123 --tractor 789`,
		Args: cobra.ExactArgs(1),
		RunE: runDoGeotabVehiclesUpdate,
	}
	initDoGeotabVehiclesUpdateFlags(cmd)
	return cmd
}

func init() {
	doGeotabVehiclesCmd.AddCommand(newDoGeotabVehiclesUpdateCmd())
}

func initDoGeotabVehiclesUpdateFlags(cmd *cobra.Command) {
...
func runDoGeotabVehiclesUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoGeotabVehiclesUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/geotab-vehicles/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated geotab vehicle %s\n", resp.Data.ID)
	return nil
}

func parseDoGeotabVehiclesUpdateOptions(cmd *cobra.Command, args []string) (doGeotabVehiclesUpdateOptions, error) {
	...
	return doGeotabVehiclesUpdateOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		ID:      args[0],
