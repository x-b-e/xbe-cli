...
type temedaVehiclesListOptions struct {
	BaseURL               string
	Token                 string
	JSON                  bool
	NoAuth                bool
...
func newTemedaVehiclesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List temeda vehicles",
		Long: `List temeda vehicles with filtering and pagination.
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
		Example: `  # List temeda vehicles
  xbe view temeda-vehicles list

  # Filter by broker
  xbe view temeda-vehicles list --broker 123

  # Filter by assignment time
  xbe view temeda-vehicles list --assigned-at-min "2024-01-01T00:00:00Z"

  # Output as JSON
  xbe view temeda-vehicles list --json`,
		Args: cobra.NoArgs,
		RunE: runTemedaVehiclesList,
	}
	initTemedaVehiclesListFlags(cmd)
	return cmd
}

func init() {
	temedaVehiclesCmd.AddCommand(newTemedaVehiclesListCmd())
}

func initTemedaVehiclesListFlags(cmd *cobra.Command) {
...
func runTemedaVehiclesList(cmd *cobra.Command, _ []string) error {
	opts, err := parseTemedaVehiclesListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/temeda-vehicles", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildTemedaVehicleRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderTemedaVehiclesTable(cmd, rows)
}

func parseTemedaVehiclesListOptions(cmd *cobra.Command) (temedaVehiclesListOptions, error) {
	...
	return temedaVehiclesListOptions{
		BaseURL:               baseURL,
		Token:                 token,
		JSON:                  jsonOut,
		NoAuth:                noAuth,
...
func buildTemedaVehicleRows(resp jsonAPIResponse) []temedaVehicleRow {
	rows := make([]temedaVehicleRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		row := temedaVehicleRow{
			ID:                    resource.ID,
...
func renderTemedaVehiclesTable(cmd *cobra.Command, rows []temedaVehicleRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No temeda vehicles found.")
		return nil
	}
