...
type doLiabilityIncidentsUpdateOptions struct {
	BaseURL                string
	Token                  string
	JSON                   bool
	ID                     string
...
func newDoLiabilityIncidentsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a liability incident",
		Long: `Update a liability incident.
...
Updatable fields:
  ...
  --new-type                 Change incident type (e.g., LiabilityIncident)
...
Relationship updates:
  ...
  --parent-type              Parent incident type (e.g., liability-incidents)

Global flags (see xbe --help): --json, --base-url, --token`,
		Example: `  # Update status
  xbe do liability-incidents update 123 --status closed

  # Update net impact values
  xbe do liability-incidents update 123 --net-impact-minutes 90 --net-impact-dollars 1500

  # Change incident type
  xbe do liability-incidents update 123 --new-type LiabilityIncident`,
		Args: cobra.ExactArgs(1),
		RunE: runDoLiabilityIncidentsUpdate,
	}
	initDoLiabilityIncidentsUpdateFlags(cmd)
	return cmd
}

func init() {
	doLiabilityIncidentsCmd.AddCommand(newDoLiabilityIncidentsUpdateCmd())
}

func initDoLiabilityIncidentsUpdateFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("new-type", "", "Change incident type (e.g., LiabilityIncident)")
...
func runDoLiabilityIncidentsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoLiabilityIncidentsUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/liability-incidents/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated liability incident %s\n", row.ID)
	return nil
}

func parseDoLiabilityIncidentsUpdateOptions(cmd *cobra.Command, args []string) (doLiabilityIncidentsUpdateOptions, error) {
	...
	return doLiabilityIncidentsUpdateOptions{
		BaseURL:                baseURL,
		Token:                  token,
		JSON:                   jsonOut,
		ID:                     args[0],
