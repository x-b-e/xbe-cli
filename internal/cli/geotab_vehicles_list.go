...
type geotabVehiclesListOptions struct {
	BaseURL               string
	Token                 string
	JSON                  bool
	NoAuth                bool
...
func newGeotabVehiclesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List geotab vehicles",
		Long: `List geotab vehicles with filtering and pagination.
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
		Example: `  # List geotab vehicles
  xbe view geotab-vehicles list

  # Filter by broker
  xbe view geotab-vehicles list --broker 123

  # Filter by assignment time
  xbe view geotab-vehicles list --assigned-at-min "2024-01-01T00:00:00Z"

  # Output as JSON
  xbe view geotab-vehicles list --json`,
		Args: cobra.NoArgs,
		RunE: runGeotabVehiclesList,
	}
	initGeotabVehiclesListFlags(cmd)
	return cmd
}

func init() {
	geotabVehiclesCmd.AddCommand(newGeotabVehiclesListCmd())
}

func initGeotabVehiclesListFlags(cmd *cobra.Command) {
...
func runGeotabVehiclesList(cmd *cobra.Command, _ []string) error {
	opts, err := parseGeotabVehiclesListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/geotab-vehicles", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildGeotabVehicleRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderGeotabVehiclesTable(cmd, rows)
}

func parseGeotabVehiclesListOptions(cmd *cobra.Command) (geotabVehiclesListOptions, error) {
	...
	return geotabVehiclesListOptions{
		BaseURL:               baseURL,
		Token:                 token,
		JSON:                  jsonOut,
		NoAuth:                noAuth,
...
func buildGeotabVehicleRows(resp jsonAPIResponse) []geotabVehicleRow {
	rows := make([]geotabVehicleRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		row := geotabVehicleRow{
			ID:                    resource.ID,
...
func renderGeotabVehiclesTable(cmd *cobra.Command, rows []geotabVehicleRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No geotab vehicles found.")
		return nil
	}
