...
type brokerTenderCancelledSellerNotificationsListOptions struct {
	BaseURL                  string
	Token                    string
	JSON                     bool
	NoAuth                   bool
...
type brokerTenderCancelledSellerNotificationRow struct {
	ID        string `json:"id"`
...
func newBrokerTenderCancelledSellerNotificationsListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List broker tender cancelled seller notifications",
		Long: `List broker tender cancelled seller notifications with filtering and pagination.
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
  ...
  # Filter by user
  xbe view broker-tender-cancelled-seller-notifications list --user 123

  # Output as JSON
  xbe view broker-tender-cancelled-seller-notifications list --json`,
		Args: cobra.NoArgs,
		RunE: runBrokerTenderCancelledSellerNotificationsList,
	}
	initBrokerTenderCancelledSellerNotificationsListFlags(cmd)
	return cmd
}

func init() {
	brokerTenderCancelledSellerNotificationsCmd.AddCommand(newBrokerTenderCancelledSellerNotificationsListCmd())
}

func initBrokerTenderCancelledSellerNotificationsListFlags(cmd *cobra.Command) {
...
func runBrokerTenderCancelledSellerNotificationsList(cmd *cobra.Command, _ []string) error {
	opts, err := parseBrokerTenderCancelledSellerNotificationsListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/broker-tender-cancelled-seller-notifications", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildBrokerTenderCancelledSellerNotificationRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderBrokerTenderCancelledSellerNotificationsTable(cmd, rows)
}

func parseBrokerTenderCancelledSellerNotificationsListOptions(cmd *cobra.Command) (brokerTenderCancelledSellerNotificationsListOptions, error) {
	...
	return brokerTenderCancelledSellerNotificationsListOptions{
		BaseURL:                  baseURL,
		Token:                    token,
		JSON:                     jsonOut,
		NoAuth:                   noAuth,
...
func buildBrokerTenderCancelledSellerNotificationRows(resp jsonAPIResponse) []brokerTenderCancelledSellerNotificationRow {
	rows := make([]brokerTenderCancelledSellerNotificationRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		attrs := resource.Attributes
		row := brokerTenderCancelledSellerNotificationRow{
			ID:        resource.ID,
...
func renderBrokerTenderCancelledSellerNotificationsTable(cmd *cobra.Command, rows []brokerTenderCancelledSellerNotificationRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No broker tender cancelled seller notifications found.")
		return nil
	}
