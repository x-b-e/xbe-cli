...
type doCustomerMembershipsCreateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	User                                 string
...
func newDoCustomerMembershipsCreateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a customer membership",
		Long: `Create a new customer membership.
...
Notification flags (true/false):
  ...
  xbe do customer-memberships create \
    --user 123 \
    --customer 456 \
    --kind manager \
    --title "Dispatch" \
  ...
  # Get JSON output
  xbe do customer-memberships create --user 123 --customer 456 --json`,
		Args: cobra.NoArgs,
		RunE: runDoCustomerMembershipsCreate,
	}
	initDoCustomerMembershipsCreateFlags(cmd)
	return cmd
}

func init() {
	doCustomerMembershipsCmd.AddCommand(newDoCustomerMembershipsCreateCmd())
}

func initDoCustomerMembershipsCreateFlags(cmd *cobra.Command) {
...
func runDoCustomerMembershipsCreate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoCustomerMembershipsCreateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Post(cmd.Context(), "/v1/customer-memberships", jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Created customer membership %s\n\n", details.ID)
	return renderMembershipDetails(cmd, details)
}

func parseDoCustomerMembershipsCreateOptions(cmd *cobra.Command) (doCustomerMembershipsCreateOptions, error) {
	...
	return doCustomerMembershipsCreateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		User:                                 user,
