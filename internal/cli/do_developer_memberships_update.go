...
type doDeveloperMembershipsUpdateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	Kind                                 string
...
func newDoDeveloperMembershipsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a developer membership",
		Long: `Update an existing developer membership.

Only the fields you specify will be updated. Fields not provided remain unchanged.
Note: User and developer cannot be changed after creation.
...
Global flags (see xbe --help): --json, --base-url, --token`,
		Example: `  # Update role
  xbe do developer-memberships update 686 --kind manager

  # Update multiple fields
  xbe do developer-memberships update 686 --is-admin true --title "Manager"

  # Update permissions
  xbe do developer-memberships update 686 --can-see-rates-as-manager false

  # Get JSON output
  xbe do developer-memberships update 686 --kind manager --json`,
		Args: cobra.ExactArgs(1),
		RunE: runDoDeveloperMembershipsUpdate,
	}
	initDoDeveloperMembershipsUpdateFlags(cmd)
	return cmd
}

func init() {
	doDeveloperMembershipsCmd.AddCommand(newDoDeveloperMembershipsUpdateCmd())
}

func initDoDeveloperMembershipsUpdateFlags(cmd *cobra.Command) {
...
func runDoDeveloperMembershipsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoDeveloperMembershipsUpdateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/developer-memberships/"+id, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated developer membership %s\n\n", details.ID)
	return renderDeveloperMembershipDetails(cmd, details)
}

func parseDoDeveloperMembershipsUpdateOptions(cmd *cobra.Command) (doDeveloperMembershipsUpdateOptions, error) {
	...
	flagNames := []string{
		...
		"is-default-job-production-plan-subscriber", "enable-inventory-capacity-notifications",
	}
	...
	return doDeveloperMembershipsUpdateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		Kind:                                 kind,
