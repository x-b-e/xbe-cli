...
type doBrokerSettingsUpdateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	ID                                   string
	...
	RestrictContactInfoVisibility        bool
	RequireExplicitRateEditingPermission bool
}

func newDoBrokerSettingsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update broker settings",
		Long: `Update broker settings.
...
Common flags:
  ...
  --restrict-contact-info-visibility      Restrict contact info visibility
  --require-explicit-rate-editing-permission  Require explicit rate editing permission`,
		Example: `  # Enable recap notifications
  xbe do broker-settings update 123 --enable-recap-notifications

  # Require project for plans
  xbe do broker-settings update 123 --plan-requires-project`,
		Args: cobra.ExactArgs(1),
		RunE: runDoBrokerSettingsUpdate,
	}
	initDoBrokerSettingsUpdateFlags(cmd)
	return cmd
}

func init() {
	doBrokerSettingsCmd.AddCommand(newDoBrokerSettingsUpdateCmd())
}

func initDoBrokerSettingsUpdateFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Bool("restrict-contact-info-visibility", false, "Restrict contact info visibility")
	cmd.Flags().Bool("require-explicit-rate-editing-permission", false, "Require explicit rate editing permission")
	cmd.Flags().String("base-url", defaultBaseURL(), "API base URL")
	cmd.Flags().String("token", "", "API token (optional)")
}

func runDoBrokerSettingsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoBrokerSettingsUpdateOptions(cmd, args)
	if err != nil {
	...
	if cmd.Flags().Changed("restrict-contact-info-visibility") {
		attributes["restrict-contact-info-visibility"] = opts.RestrictContactInfoVisibility
	}
	if cmd.Flags().Changed("require-explicit-rate-editing-permission") {
		attributes["require-explicit-rate-editing-permission"] = opts.RequireExplicitRateEditingPermission
	}
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/broker-settings/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	if opts.JSON {
		row := brokerSettingRow{
			...
			RestrictContactInfoVisibility:        boolAttr(resp.Data.Attributes, "restrict-contact-info-visibility"),
			RequireExplicitRateEditingPermission: boolAttr(resp.Data.Attributes, "require-explicit-rate-editing-permission"),
		}
		return writeJSON(cmd.OutOrStdout(), row)
	}

	fmt.Fprintf(cmd.OutOrStdout(), "Updated broker settings %s\n", resp.Data.ID)
	return nil
}

func parseDoBrokerSettingsUpdateOptions(cmd *cobra.Command, args []string) (doBrokerSettingsUpdateOptions, error) {
	...
	restrictContactInfoVisibility, _ := cmd.Flags().GetBool("restrict-contact-info-visibility")
	requireExplicitRateEditingPermission, _ := cmd.Flags().GetBool("require-explicit-rate-editing-permission")
	baseURL, _ := cmd.Flags().GetString("base-url")
	token, _ := cmd.Flags().GetString("token")

	return doBrokerSettingsUpdateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		ID:                                   args[0],
		...
		RestrictContactInfoVisibility:        restrictContactInfoVisibility,
		RequireExplicitRateEditingPermission: requireExplicitRateEditingPermission,
	}, nil
}
