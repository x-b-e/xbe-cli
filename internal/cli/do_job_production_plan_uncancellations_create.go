...
type doJobProductionPlanUncancellationsCreateOptions struct {
	BaseURL                           string
	Token                             string
	JSON                              bool
...
type jobProductionPlanUncancellationRow struct {
	ID                                string `json:"id"`
...
func newDoJobProductionPlanUncancellationsCreateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Uncancel a job production plan",
		Long: `Uncancel a job production plan.
...
Optional:
  ...
  xbe do job-production-plan-uncancellations create \
    ...
    --suppress-status-change-notifications`,
		Args: cobra.NoArgs,
		RunE: runDoJobProductionPlanUncancellationsCreate,
	}
	initDoJobProductionPlanUncancellationsCreateFlags(cmd)
	return cmd
}

func init() {
	doJobProductionPlanUncancellationsCmd.AddCommand(newDoJobProductionPlanUncancellationsCreateCmd())
}

func initDoJobProductionPlanUncancellationsCreateFlags(cmd *cobra.Command) {
...
func runDoJobProductionPlanUncancellationsCreate(cmd *cobra.Command, _ []string) error {
	opts, err := parseDoJobProductionPlanUncancellationsCreateOptions(cmd)
	if err != nil {
	...
	data := map[string]any{
		"type":          "job-production-plan-uncancellations",
	...
	body, _, err := client.Post(cmd.Context(), "/v1/job-production-plan-uncancellations", jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	row := buildJobProductionPlanUncancellationRowFromSingle(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), row)
	}

	fmt.Fprintf(cmd.OutOrStdout(), "Created job production plan uncancellation %s\n", row.ID)
	return nil
}

func parseDoJobProductionPlanUncancellationsCreateOptions(cmd *cobra.Command) (doJobProductionPlanUncancellationsCreateOptions, error) {
	...
	return doJobProductionPlanUncancellationsCreateOptions{
		BaseURL:                           baseURL,
		Token:                             token,
		JSON:                              jsonOut,
...
func buildJobProductionPlanUncancellationRowFromSingle(resp jsonAPISingleResponse) jobProductionPlanUncancellationRow {
	resource := resp.Data
	attrs := resource.Attributes
	row := jobProductionPlanUncancellationRow{
		ID:                                resource.ID,
