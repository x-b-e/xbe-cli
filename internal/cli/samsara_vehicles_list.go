...
type samsaraVehiclesListOptions struct {
	BaseURL               string
	Token                 string
	JSON                  bool
	NoAuth                bool
...
func newSamsaraVehiclesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List samsara vehicles",
		Long: `List samsara vehicles with filtering and pagination.
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
		Example: `  # List samsara vehicles
  xbe view samsara-vehicles list

  # Filter by broker
  xbe view samsara-vehicles list --broker 123

  # Filter by assignment time
  xbe view samsara-vehicles list --assigned-at-min "2024-01-01T00:00:00Z"

  # Output as JSON
  xbe view samsara-vehicles list --json`,
		Args: cobra.NoArgs,
		RunE: runSamsaraVehiclesList,
	}
	initSamsaraVehiclesListFlags(cmd)
	return cmd
}

func init() {
	samsaraVehiclesCmd.AddCommand(newSamsaraVehiclesListCmd())
}

func initSamsaraVehiclesListFlags(cmd *cobra.Command) {
...
func runSamsaraVehiclesList(cmd *cobra.Command, _ []string) error {
	opts, err := parseSamsaraVehiclesListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/samsara-vehicles", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildSamsaraVehicleRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderSamsaraVehiclesTable(cmd, rows)
}

func parseSamsaraVehiclesListOptions(cmd *cobra.Command) (samsaraVehiclesListOptions, error) {
	...
	return samsaraVehiclesListOptions{
		BaseURL:               baseURL,
		Token:                 token,
		JSON:                  jsonOut,
		NoAuth:                noAuth,
...
func buildSamsaraVehicleRows(resp jsonAPIResponse) []samsaraVehicleRow {
	rows := make([]samsaraVehicleRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		row := samsaraVehicleRow{
			ID:                    resource.ID,
...
func renderSamsaraVehiclesTable(cmd *cobra.Command, rows []samsaraVehicleRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No samsara vehicles found.")
		return nil
	}
