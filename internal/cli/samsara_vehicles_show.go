...
type samsaraVehiclesShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type samsaraVehicleDetails struct {
	ID                    string `json:"id"`
...
func newSamsaraVehiclesShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show samsara vehicle details",
		Long: `Show the full details of a samsara vehicle.

Output Fields:
  ID                 Samsara vehicle identifier
  VEHICLE NUMBER     Samsara vehicle number
  VEHICLE ID         Samsara vehicle external ID
...
Arguments:
  <id>    The samsara vehicle ID (required). Use the list command to find IDs.

Global flags (see xbe --help): --json, --base-url, --token, --no-auth`,
		Example: `  # Show a samsara vehicle
  xbe view samsara-vehicles show 123

  # Output as JSON
  xbe view samsara-vehicles show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runSamsaraVehiclesShow,
	}
	initSamsaraVehiclesShowFlags(cmd)
	return cmd
}

func init() {
	samsaraVehiclesCmd.AddCommand(newSamsaraVehiclesShowCmd())
}

func initSamsaraVehiclesShowFlags(cmd *cobra.Command) {
...
func runSamsaraVehiclesShow(cmd *cobra.Command, args []string) error {
	opts, err := parseSamsaraVehiclesShowOptions(cmd)
	if err != nil {
	...
	if id == "" {
		return fmt.Errorf("samsara vehicle id is required")
	}

	client := api.NewClient(opts.BaseURL, opts.Token)

	query := url.Values{}
	query.Set("fields[samsara-vehicles]", "vehicle-id,vehicle-number,integration-identifier,trailer-set-at,tractor-set-at,broker,trucker,trailer,tractor")
	query.Set("include", "broker,trucker,trailer,tractor")

	body, _, err := client.Get(cmd.Context(), "/v1/samsara-vehicles/"+id, query)
	if err != nil {
		if len(body) > 0 {
	...
	details := buildSamsaraVehicleDetails(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), details)
	}

	return renderSamsaraVehicleDetails(cmd, details)
}

func parseSamsaraVehiclesShowOptions(cmd *cobra.Command) (samsaraVehiclesShowOptions, error) {
	jsonOut, err := cmd.Flags().GetBool("json")
	if err != nil {
		return samsaraVehiclesShowOptions{}, err
	}
	...
	return samsaraVehiclesShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildSamsaraVehicleDetails(resp jsonAPISingleResponse) samsaraVehicleDetails {
	attrs := resp.Data.Attributes
	return samsaraVehicleDetails{
		ID:                    resp.Data.ID,
...
func renderSamsaraVehicleDetails(cmd *cobra.Command, details samsaraVehicleDetails) error {
	out := cmd.OutOrStdout()
