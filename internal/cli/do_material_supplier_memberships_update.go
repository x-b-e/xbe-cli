...
type doMaterialSupplierMembershipsUpdateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	Kind                                 string
...
func newDoMaterialSupplierMembershipsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a material supplier membership",
		Long: `Update an existing material supplier membership.

Only the fields you specify will be updated. Fields not provided remain unchanged.
Note: User and material supplier cannot be changed after creation.
...
Notes:
  ...
  xbe do material-supplier-memberships update 686 --kind operations

  # Update multiple fields
  xbe do material-supplier-memberships update 686 --kind manager --is-admin true --title "Manager"

  # Update notifications
  xbe do material-supplier-memberships update 686 --enable-recap-notifications true

  # Get JSON output
  xbe do material-supplier-memberships update 686 --kind manager --json`,
		Args: cobra.ExactArgs(1),
		RunE: runDoMaterialSupplierMembershipsUpdate,
	}
	initDoMaterialSupplierMembershipsUpdateFlags(cmd)
	return cmd
}

func init() {
	doMaterialSupplierMembershipsCmd.AddCommand(newDoMaterialSupplierMembershipsUpdateCmd())
}

func initDoMaterialSupplierMembershipsUpdateFlags(cmd *cobra.Command) {
...
func runDoMaterialSupplierMembershipsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoMaterialSupplierMembershipsUpdateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/material-supplier-memberships/"+id, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated material supplier membership %s\n\n", details.ID)
	return renderMembershipDetails(cmd, details)
}

func parseDoMaterialSupplierMembershipsUpdateOptions(cmd *cobra.Command) (doMaterialSupplierMembershipsUpdateOptions, error) {
	flagsSet := make(map[string]bool)
	flagNames := []string{
		"kind", "is-admin", "title", "color-hex", "external-employee-id",
		"explicit-sort-order", "start-at", "end-at", "drives-shift-type",
		"project-office", "can-see-rates-as-manager", "can-validate-profit-improvements",
	...
	return doMaterialSupplierMembershipsUpdateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		Kind:                                 kind,
