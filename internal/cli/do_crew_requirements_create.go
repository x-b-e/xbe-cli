...
type doCrewRequirementsCreateOptions struct {
	BaseURL                                  string
	Token                                    string
	JSON                                     bool
...
func newDoCrewRequirementsCreateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a crew requirement",
		Long: `Create a crew requirement.

Required flags:
  --requirement-type              Requirement type (labor or equipment) (required)
  --job-production-plan           Job production plan ID (required)
  --resource-classification-type  Resource classification type (JSON:API type, e.g., labor-classifications) (required)
  --resource-classification-id    Resource classification ID (required)

Optional flags:
  --resource-type                 Resource type (JSON:API type, e.g., laborers or equipment)
  ...
  xbe do crew-requirements create \\
    ...
    --resource-classification-id 456`,
		Args: cobra.NoArgs,
		RunE: runDoCrewRequirementsCreate,
	}
	initDoCrewRequirementsCreateFlags(cmd)
	return cmd
}

func init() {
	doCrewRequirementsCmd.AddCommand(newDoCrewRequirementsCreateCmd())
}

func initDoCrewRequirementsCreateFlags(cmd *cobra.Command) {
	cmd.Flags().Bool("json", false, "Output JSON")
	cmd.Flags().String("requirement-type", "", "Requirement type (labor or equipment) (required)")
	cmd.Flags().String("job-production-plan", "", "Job production plan ID (required)")
	cmd.Flags().String("resource-classification-type", "", "Resource classification type (JSON:API type, e.g., labor-classifications) (required)")
	cmd.Flags().String("resource-classification-id", "", "Resource classification ID (required)")
	cmd.Flags().String("resource-type", "", "Resource type (JSON:API type, e.g., laborers or equipment)")
...
func runDoCrewRequirementsCreate(cmd *cobra.Command, _ []string) error {
	opts, err := parseDoCrewRequirementsCreateOptions(cmd)
	if err != nil {
	...
	if opts.RequirementType == "" {
		return fmt.Errorf("--requirement-type is required")
	}
	...
	resourceType, endpoint, err := parseCrewRequirementKind(opts.RequirementType)
	if err != nil {
		fmt.Fprintln(cmd.ErrOrStderr(), err)
		return err
	}
	...
	row := buildCrewRequirementRowFromSingle(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), row)
	}

	fmt.Fprintf(cmd.OutOrStdout(), "Created crew requirement %s\n", row.ID)
	return nil
}

func parseDoCrewRequirementsCreateOptions(cmd *cobra.Command) (doCrewRequirementsCreateOptions, error) {
	...
	return doCrewRequirementsCreateOptions{
		BaseURL:                                  baseURL,
		Token:                                    token,
		JSON:                                     jsonOut,
