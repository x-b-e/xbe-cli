...
type doEfficiencyIncidentsUpdateOptions struct {
	BaseURL                string
	Token                  string
	JSON                   bool
	ID                     string
...
func newDoEfficiencyIncidentsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update an efficiency incident",
		Long: `Update an existing efficiency incident.

Only the fields you specify will be updated. Fields not provided remain unchanged.

Arguments:
  <id>    The efficiency incident ID (required)
...
Flags:
  ...
  --kind                      Update kind (over_trucking)
...
Global flags (see xbe --help): --json, --base-url, --token`,
  ...
  xbe do efficiency-incidents update 123 --net-impact-dollars 1500`,
		Args: cobra.ExactArgs(1),
		RunE: runDoEfficiencyIncidentsUpdate,
	}
	initDoEfficiencyIncidentsUpdateFlags(cmd)
	return cmd
}

func init() {
	doEfficiencyIncidentsCmd.AddCommand(newDoEfficiencyIncidentsUpdateCmd())
}

func initDoEfficiencyIncidentsUpdateFlags(cmd *cobra.Command) {
...
func runDoEfficiencyIncidentsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoEfficiencyIncidentsUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/efficiency-incidents/"+id, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated efficiency incident %s\n", details.ID)
	return renderEfficiencyIncidentDetails(cmd, details)
}

func parseDoEfficiencyIncidentsUpdateOptions(cmd *cobra.Command, args []string) (doEfficiencyIncidentsUpdateOptions, error) {
	...
	return doEfficiencyIncidentsUpdateOptions{
		BaseURL:                baseURL,
		Token:                  token,
		JSON:                   jsonOut,
		ID:                     args[0],
