...
type doCrewRequirementsUpdateOptions struct {
	BaseURL                                  string
	Token                                    string
	JSON                                     bool
	ID                                       string
...
func newDoCrewRequirementsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a crew requirement",
		Long: `Update a crew requirement.

Provide at least one attribute or relationship to update.`,
		Example: `  # Update note and mobilization method
  xbe do crew-requirements update 123 --note "Updated note" --mobilization-method trailer

  # Update schedule
  xbe do crew-requirements update 123 --start-at 2025-01-02T08:00:00Z --end-at 2025-01-02T16:00:00Z

  # Update resource assignment
  xbe do crew-requirements update 123 --resource-type equipment --resource-id 456`,
		Args: cobra.ExactArgs(1),
		RunE: runDoCrewRequirementsUpdate,
	}
	initDoCrewRequirementsUpdateFlags(cmd)
	return cmd
}

func init() {
	doCrewRequirementsCmd.AddCommand(newDoCrewRequirementsUpdateCmd())
}

func initDoCrewRequirementsUpdateFlags(cmd *cobra.Command) {
	cmd.Flags().Bool("json", false, "Output JSON")
	cmd.Flags().String("job-production-plan", "", "Job production plan ID")
	cmd.Flags().String("resource-classification-type", "", "Resource classification type (JSON:API type, e.g., labor-classifications)")
...
func runDoCrewRequirementsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoCrewRequirementsUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/crew-requirements/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated crew requirement %s\n", row.ID)
	return nil
}

func parseDoCrewRequirementsUpdateOptions(cmd *cobra.Command, args []string) (doCrewRequirementsUpdateOptions, error) {
	...
	return doCrewRequirementsUpdateOptions{
		BaseURL:                                  baseURL,
		Token:                                    token,
		JSON:                                     jsonOut,
		ID:                                       args[0],
