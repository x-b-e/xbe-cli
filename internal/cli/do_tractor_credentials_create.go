...
type doTractorCredentialsCreateOptions struct {
	BaseURL                                  string
	Token                                    string
	JSON                                     bool
	TractorID                                string
...
func newDoTractorCredentialsCreateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a new tractor credential",
		Long: `Create a new tractor credential.
...
Optional flags:
  ...
  --expires-on    Expiration date (YYYY-MM-DD)`,
		Example: `  # Create a tractor credential
  xbe do tractor-credentials create --tractor 123 --tractor-trailer-credential-classification 456

  # Create with dates
  xbe do tractor-credentials create --tractor 123 --tractor-trailer-credential-classification 456 --issued-on 2024-01-01 --expires-on 2025-01-01`,
		Args: cobra.NoArgs,
		RunE: runDoTractorCredentialsCreate,
	}
	initDoTractorCredentialsCreateFlags(cmd)
	return cmd
}

func init() {
	doTractorCredentialsCmd.AddCommand(newDoTractorCredentialsCreateCmd())
}

func initDoTractorCredentialsCreateFlags(cmd *cobra.Command) {
...
func runDoTractorCredentialsCreate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoTractorCredentialsCreateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Post(cmd.Context(), "/v1/tractor-credentials", jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Created tractor credential %s\n", row.ID)
	return nil
}

func parseDoTractorCredentialsCreateOptions(cmd *cobra.Command) (doTractorCredentialsCreateOptions, error) {
	...
	return doTractorCredentialsCreateOptions{
		BaseURL:                                  baseURL,
		Token:                                    token,
		JSON:                                     jsonOut,
		TractorID:                                tractorID,
...
func buildTractorCredentialRowFromSingle(resp jsonAPISingleResponse) tractorCredentialRow {
