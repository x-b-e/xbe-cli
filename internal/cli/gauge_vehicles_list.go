...
type gaugeVehiclesListOptions struct {
	BaseURL               string
	Token                 string
	JSON                  bool
	NoAuth                bool
...
func newGaugeVehiclesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List gauge vehicles",
		Long: `List gauge vehicles with filtering and pagination.
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
		Example: `  # List gauge vehicles
  xbe view gauge-vehicles list

  # Filter by broker
  xbe view gauge-vehicles list --broker 123

  # Filter by assignment time
  xbe view gauge-vehicles list --assigned-at-min "2024-01-01T00:00:00Z"

  # Output as JSON
  xbe view gauge-vehicles list --json`,
		Args: cobra.NoArgs,
		RunE: runGaugeVehiclesList,
	}
	initGaugeVehiclesListFlags(cmd)
	return cmd
}

func init() {
	gaugeVehiclesCmd.AddCommand(newGaugeVehiclesListCmd())
}

func initGaugeVehiclesListFlags(cmd *cobra.Command) {
...
func runGaugeVehiclesList(cmd *cobra.Command, _ []string) error {
	opts, err := parseGaugeVehiclesListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/gauge-vehicles", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildGaugeVehicleRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderGaugeVehiclesTable(cmd, rows)
}

func parseGaugeVehiclesListOptions(cmd *cobra.Command) (gaugeVehiclesListOptions, error) {
	...
	return gaugeVehiclesListOptions{
		BaseURL:               baseURL,
		Token:                 token,
		JSON:                  jsonOut,
		NoAuth:                noAuth,
...
func buildGaugeVehicleRows(resp jsonAPIResponse) []gaugeVehicleRow {
	rows := make([]gaugeVehicleRow, 0, len(resp.Data))
...
func renderGaugeVehiclesTable(cmd *cobra.Command, rows []gaugeVehicleRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No gauge vehicles found.")
		return nil
	}
