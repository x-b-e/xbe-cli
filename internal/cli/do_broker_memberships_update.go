...
type doBrokerMembershipsUpdateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	Kind                                 string
...
func newDoBrokerMembershipsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a broker membership",
		Long: `Update an existing broker membership.

Only the fields you specify will be updated. Fields not provided remain unchanged.
Note: User and broker cannot be changed after creation.
...
Flags:
  ...
  --title                    Title within the broker organization
...
Global flags (see xbe --help): --json, --base-url, --token`,
		Example: `  # Update role
  xbe do broker-memberships update 686 --kind manager

  # Update multiple fields
  xbe do broker-memberships update 686 --is-admin true --title "Manager"

  # Update permissions
  xbe do broker-memberships update 686 --can-see-rates-as-manager false --is-rate-editor true

  # Get JSON output
  xbe do broker-memberships update 686 --kind manager --json`,
		Args: cobra.ExactArgs(1),
		RunE: runDoBrokerMembershipsUpdate,
	}
	initDoBrokerMembershipsUpdateFlags(cmd)
	return cmd
}

func init() {
	doBrokerMembershipsCmd.AddCommand(newDoBrokerMembershipsUpdateCmd())
}

func initDoBrokerMembershipsUpdateFlags(cmd *cobra.Command) {
...
func runDoBrokerMembershipsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoBrokerMembershipsUpdateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/broker-memberships/"+id, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated broker membership %s\n\n", details.ID)
	return renderBrokerMembershipDetails(cmd, details)
}

func parseDoBrokerMembershipsUpdateOptions(cmd *cobra.Command) (doBrokerMembershipsUpdateOptions, error) {
	...
	return doBrokerMembershipsUpdateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		Kind:                                 kind,
