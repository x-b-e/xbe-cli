...
type teletracNavmanVehiclesListOptions struct {
	BaseURL               string
	Token                 string
	JSON                  bool
	NoAuth                bool
...
type teletracNavmanVehicleRow struct {
	ID                    string `json:"id"`
...
func newTeletracNavmanVehiclesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List Teletrac Navman vehicles",
		Long: `List Teletrac Navman vehicles with filtering and pagination.

Output Columns:
  ID         Teletrac Navman vehicle identifier
  VEHICLE #  Teletrac Navman vehicle number
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
  ...
  # Filter by broker
  xbe view teletrac-navman-vehicles list --broker 123

  # Filter by trailer assignment
  xbe view teletrac-navman-vehicles list --has-trailer true

  # Output as JSON
  xbe view teletrac-navman-vehicles list --json`,
		Args: cobra.NoArgs,
		RunE: runTeletracNavmanVehiclesList,
	}
	initTeletracNavmanVehiclesListFlags(cmd)
	return cmd
}

func init() {
	teletracNavmanVehiclesCmd.AddCommand(newTeletracNavmanVehiclesListCmd())
}

func initTeletracNavmanVehiclesListFlags(cmd *cobra.Command) {
...
func runTeletracNavmanVehiclesList(cmd *cobra.Command, _ []string) error {
	opts, err := parseTeletracNavmanVehiclesListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/teletrac-navman-vehicles", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildTeletracNavmanVehicleRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderTeletracNavmanVehiclesTable(cmd, rows)
}

func parseTeletracNavmanVehiclesListOptions(cmd *cobra.Command) (teletracNavmanVehiclesListOptions, error) {
	...
	return teletracNavmanVehiclesListOptions{
		BaseURL:               baseURL,
		Token:                 token,
		JSON:                  jsonOut,
		NoAuth:                noAuth,
...
func buildTeletracNavmanVehicleRows(resp jsonAPIResponse) []teletracNavmanVehicleRow {
	included := make(map[string]jsonAPIResource, len(resp.Included))
	for _, inc := range resp.Included {
		included[resourceKey(inc.Type, inc.ID)] = inc
	}

	rows := make([]teletracNavmanVehicleRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		rows = append(rows, buildTeletracNavmanVehicleRow(resource, included))
	}
	return rows
}

func teletracNavmanVehicleRowFromSingle(resp jsonAPISingleResponse) teletracNavmanVehicleRow {
	included := make(map[string]jsonAPIResource, len(resp.Included))
	for _, inc := range resp.Included {
		included[resourceKey(inc.Type, inc.ID)] = inc
	}
	return buildTeletracNavmanVehicleRow(resp.Data, included)
}

func buildTeletracNavmanVehicleRow(resource jsonAPIResource, included map[string]jsonAPIResource) teletracNavmanVehicleRow {
	attrs := resource.Attributes
	row := teletracNavmanVehicleRow{
		ID:                    resource.ID,
...
func renderTeletracNavmanVehiclesTable(cmd *cobra.Command, rows []teletracNavmanVehicleRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No Teletrac Navman vehicles found.")
		return nil
	}

	writer := tabwriter.NewWriter(cmd.OutOrStdout(), 2, 4, 2, ' ', 0)
	fmt.Fprintln(writer, "ID\tVEHICLE #\tVEHICLE ID\tTRACTOR\tTRAILER\tTRUCKER\tBROKER")
