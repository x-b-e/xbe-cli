...
type doAdministrativeIncidentsCreateOptions struct {
	BaseURL                string
	Token                  string
	JSON                   bool
	Subject                string
...
func newDoAdministrativeIncidentsCreateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create an administrative incident",
		Long: `Create a new administrative incident.
...
Optional flags:
  --kind                     Incident kind (capacity, good_catch, near_miss, planning, quality, trucking)
...
Global flags (see xbe --help): --json, --base-url, --token`,
  ...
  xbe do administrative-incidents create --subject Broker|123 --start-at 2025-01-01T08:00:00Z --status open --net-impact-dollars 1500`,
		Args: cobra.NoArgs,
		RunE: runDoAdministrativeIncidentsCreate,
	}
	initDoAdministrativeIncidentsCreateFlags(cmd)
	return cmd
}

func init() {
	doAdministrativeIncidentsCmd.AddCommand(newDoAdministrativeIncidentsCreateCmd())
}

func initDoAdministrativeIncidentsCreateFlags(cmd *cobra.Command) {
...
func runDoAdministrativeIncidentsCreate(cmd *cobra.Command, _ []string) error {
	opts, err := parseDoAdministrativeIncidentsCreateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Post(cmd.Context(), "/v1/administrative-incidents", jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Created administrative incident %s (%s)\n", details.ID, subjectClass)
	return renderAdministrativeIncidentDetails(cmd, details)
}

func parseDoAdministrativeIncidentsCreateOptions(cmd *cobra.Command) (doAdministrativeIncidentsCreateOptions, error) {
	...
	return doAdministrativeIncidentsCreateOptions{
		BaseURL:                baseURL,
		Token:                  token,
		JSON:                   jsonOut,
		Subject:                subject,
