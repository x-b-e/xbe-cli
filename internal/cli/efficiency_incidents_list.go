...
type efficiencyIncidentsListOptions struct {
	BaseURL                       string
	Token                         string
	JSON                          bool
	NoAuth                        bool
...
type efficiencyIncidentRow struct {
	ID                string `json:"id"`
...
func newEfficiencyIncidentsListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List efficiency incidents",
		Long: `List efficiency incidents with filtering and pagination.
...
Filters:
  ...
  --kind                           Filter by kind (comma-separated: over_trucking)
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
  ...
  xbe view efficiency-incidents list --status open --kind over_trucking

  # Filter by subject and date range
  xbe view efficiency-incidents list --subject Broker|123 --start-on-min 2024-01-01 --start-on-max 2024-12-31

  # Output as JSON
  xbe view efficiency-incidents list --json`,
		Args: cobra.NoArgs,
		RunE: runEfficiencyIncidentsList,
	}
	initEfficiencyIncidentsListFlags(cmd)
	return cmd
}

func init() {
	efficiencyIncidentsCmd.AddCommand(newEfficiencyIncidentsListCmd())
}

func initEfficiencyIncidentsListFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("kind", "", "Filter by kind (comma-separated: over_trucking)")
...
func runEfficiencyIncidentsList(cmd *cobra.Command, _ []string) error {
	opts, err := parseEfficiencyIncidentsListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/efficiency-incidents", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildEfficiencyIncidentRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderEfficiencyIncidentsTable(cmd, rows)
}

func parseEfficiencyIncidentsListOptions(cmd *cobra.Command) (efficiencyIncidentsListOptions, error) {
	...
	return efficiencyIncidentsListOptions{
		BaseURL:                       baseURL,
		Token:                         token,
		JSON:                          jsonOut,
		NoAuth:                        noAuth,
...
func buildEfficiencyIncidentRows(resp jsonAPIResponse) []efficiencyIncidentRow {
	rows := make([]efficiencyIncidentRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		row := efficiencyIncidentRow{
			ID:               resource.ID,
...
func renderEfficiencyIncidentsTable(cmd *cobra.Command, rows []efficiencyIncidentRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No efficiency incidents found.")
		return nil
	}
