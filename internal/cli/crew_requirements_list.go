...
type crewRequirementsListOptions struct {
	BaseURL                      string
	Token                        string
	JSON                         bool
	NoAuth                       bool
...
type crewRequirementRow struct {
	ID                         string `json:"id"`
...
func newCrewRequirementsListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List crew requirements",
		Long: `List crew requirements.

Output Columns:
  ID           Crew requirement identifier
  TYPE         Requirement type (LaborRequirement, EquipmentRequirement)
  JPP          Job production plan ID
  CLASS        Resource classification type/ID
  RESOURCE     Assigned resource type/ID
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
		Example: `  # List crew requirements
  ...
  xbe view crew-requirements list --resource-classification-type LaborClassification --resource-classification-id 456

  # Filter by start time range
  xbe view crew-requirements list --start-at-min 2025-01-01T00:00:00Z --start-at-max 2025-01-31T23:59:59Z

  # Output as JSON
  xbe view crew-requirements list --json`,
		Args: cobra.NoArgs,
		RunE: runCrewRequirementsList,
	}
	initCrewRequirementsListFlags(cmd)
	return cmd
}

func init() {
	crewRequirementsCmd.AddCommand(newCrewRequirementsListCmd())
}

func initCrewRequirementsListFlags(cmd *cobra.Command) {
...
func runCrewRequirementsList(cmd *cobra.Command, _ []string) error {
	opts, err := parseCrewRequirementsListOptions(cmd)
	if err != nil {
	...
	query := url.Values{}
	query.Set("fields[crew-requirements]", "polymorphic-type,start-at,end-at,mobilization-method,requires-inbound-movement,requires-outbound-movement,job-production-plan,resource-classification,resource")
	...
	body, _, err := client.Get(cmd.Context(), "/v1/crew-requirements", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildCrewRequirementRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderCrewRequirementsTable(cmd, rows)
}

func parseCrewRequirementsListOptions(cmd *cobra.Command) (crewRequirementsListOptions, error) {
	...
	return crewRequirementsListOptions{
		BaseURL:                      baseURL,
		Token:                        token,
		JSON:                         jsonOut,
		NoAuth:                       noAuth,
...
func buildCrewRequirementRows(resp jsonAPIResponse) []crewRequirementRow {
	rows := make([]crewRequirementRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		rows = append(rows, buildCrewRequirementRow(resource))
	}
	return rows
}

func buildCrewRequirementRow(resource jsonAPIResource) crewRequirementRow {
	attrs := resource.Attributes
	row := crewRequirementRow{
		ID:                       resource.ID,
...
func renderCrewRequirementsTable(cmd *cobra.Command, rows []crewRequirementRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No crew requirements found.")
		return nil
	}

	writer := tabwriter.NewWriter(cmd.OutOrStdout(), 2, 4, 2, ' ', 0)
	fmt.Fprintln(writer, "ID\tTYPE\tJPP\tCLASS\tRESOURCE\tSTART AT\tEND AT\tMOBILIZATION\tINBOUND\tOUTBOUND")
	for _, row := range rows {
		...
		fmt.Fprintf(writer, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%t\t%t\n",
			row.ID,
			row.PolymorphicType,
...
func buildCrewRequirementRowFromSingle(resp jsonAPISingleResponse) crewRequirementRow {
	return buildCrewRequirementRow(resp.Data)
}
