...
type doCustomerMembershipsUpdateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	Kind                                 string
...
func newDoCustomerMembershipsUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a customer membership",
		Long: `Update an existing customer membership.

Only the fields you specify will be updated. Fields not provided remain unchanged.
Note: User and customer cannot be changed after creation.
...
Notification flags (true/false):
  ...
  xbe do customer-memberships update 686 --kind operations

  # Update multiple fields
  xbe do customer-memberships update 686 --kind manager --is-admin true --title "Manager"

  # Update permissions
  xbe do customer-memberships update 686 --can-see-rates-as-manager true

  # Get JSON output
  xbe do customer-memberships update 686 --kind manager --json`,
		Args: cobra.ExactArgs(1),
		RunE: runDoCustomerMembershipsUpdate,
	}
	initDoCustomerMembershipsUpdateFlags(cmd)
	return cmd
}

func init() {
	doCustomerMembershipsCmd.AddCommand(newDoCustomerMembershipsUpdateCmd())
}

func initDoCustomerMembershipsUpdateFlags(cmd *cobra.Command) {
...
func runDoCustomerMembershipsUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoCustomerMembershipsUpdateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/customer-memberships/"+id, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated customer membership %s\n\n", details.ID)
	return renderMembershipDetails(cmd, details)
}

func parseDoCustomerMembershipsUpdateOptions(cmd *cobra.Command) (doCustomerMembershipsUpdateOptions, error) {
	...
	return doCustomerMembershipsUpdateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		Kind:                                 kind,
