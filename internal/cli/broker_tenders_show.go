...
type brokerTendersShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type brokerTenderDetails struct {
	ID                                 string `json:"id"`
...
func newBrokerTendersShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show broker tender details",
		Long: `Show the full details of a broker tender.

Output Fields:
  ID, status, expiration, and note
  Payment and travel settings
  Job, broker, and trucker relationships
  Contact relationships
  Rates, shifts, time cards, and certification requirements

Arguments:
  <id>    The broker tender ID (required). You can find IDs using the list command.

Global flags (see xbe --help): --json, --base-url, --token, --no-auth`,
		Example: `  # Show a broker tender
  xbe view broker-tenders show 123

  # JSON output
  xbe view broker-tenders show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runBrokerTendersShow,
	}
	initBrokerTendersShowFlags(cmd)
	return cmd
}

func init() {
	brokerTendersCmd.AddCommand(newBrokerTendersShowCmd())
}

func initBrokerTendersShowFlags(cmd *cobra.Command) {
...
func runBrokerTendersShow(cmd *cobra.Command, args []string) error {
	...
	opts, err := parseBrokerTendersShowOptions(cmd)
	if err != nil {
	...
	query := url.Values{}
	query.Set("fields[broker-tenders]", strings.Join([]string{
		"status",
		"expires-at",
		"note",
	...
	query.Set("include", "job,buyer,seller,trucker,seller-financial-contact,seller-operations-contact,buyer-operations-contact,buyer-financial-contact")

	body, _, err := client.Get(cmd.Context(), "/v1/broker-tenders/"+id, query)
	if err != nil {
		if len(body) > 0 {
	...
	details := buildBrokerTenderDetails(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), details)
	}

	return renderBrokerTenderDetails(cmd, details)
}

func parseBrokerTendersShowOptions(cmd *cobra.Command) (brokerTendersShowOptions, error) {
	jsonOut, _ := cmd.Flags().GetBool("json")
	noAuth, _ := cmd.Flags().GetBool("no-auth")
	baseURL, _ := cmd.Flags().GetString("base-url")
	token, _ := cmd.Flags().GetString("token")

	return brokerTendersShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildBrokerTenderDetails(resp jsonAPISingleResponse) brokerTenderDetails {
	resource := resp.Data
	attrs := resource.Attributes

	details := brokerTenderDetails{
		ID:                                 resource.ID,
...
func renderBrokerTenderDetails(cmd *cobra.Command, details brokerTenderDetails) error {
	out := cmd.OutOrStdout()
