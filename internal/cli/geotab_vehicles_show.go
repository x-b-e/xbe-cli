...
type geotabVehiclesShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type geotabVehicleDetails struct {
	ID                    string `json:"id"`
...
func newGeotabVehiclesShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show geotab vehicle details",
		Long: `Show the full details of a geotab vehicle.

Output Fields:
  ID                 Geotab vehicle identifier
  VEHICLE NUMBER     Geotab vehicle number
  VEHICLE ID         Geotab vehicle external ID
...
Arguments:
  <id>    The geotab vehicle ID (required). Use the list command to find IDs.

Global flags (see xbe --help): --json, --base-url, --token, --no-auth`,
		Example: `  # Show a geotab vehicle
  xbe view geotab-vehicles show 123

  # Output as JSON
  xbe view geotab-vehicles show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runGeotabVehiclesShow,
	}
	initGeotabVehiclesShowFlags(cmd)
	return cmd
}

func init() {
	geotabVehiclesCmd.AddCommand(newGeotabVehiclesShowCmd())
}

func initGeotabVehiclesShowFlags(cmd *cobra.Command) {
...
func runGeotabVehiclesShow(cmd *cobra.Command, args []string) error {
	opts, err := parseGeotabVehiclesShowOptions(cmd)
	if err != nil {
	...
	if id == "" {
		return fmt.Errorf("geotab vehicle id is required")
	}

	client := api.NewClient(opts.BaseURL, opts.Token)

	query := url.Values{}
	query.Set("fields[geotab-vehicles]", "vehicle-id,vehicle-number,integration-identifier,trailer-set-at,tractor-set-at,broker,trucker,trailer,tractor")
	query.Set("include", "broker,trucker,trailer,tractor")

	body, _, err := client.Get(cmd.Context(), "/v1/geotab-vehicles/"+id, query)
	if err != nil {
		if len(body) > 0 {
	...
	details := buildGeotabVehicleDetails(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), details)
	}

	return renderGeotabVehicleDetails(cmd, details)
}

func parseGeotabVehiclesShowOptions(cmd *cobra.Command) (geotabVehiclesShowOptions, error) {
	jsonOut, err := cmd.Flags().GetBool("json")
	if err != nil {
		return geotabVehiclesShowOptions{}, err
	}
	...
	return geotabVehiclesShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildGeotabVehicleDetails(resp jsonAPISingleResponse) geotabVehicleDetails {
	attrs := resp.Data.Attributes
	return geotabVehicleDetails{
		ID:                    resp.Data.ID,
...
func renderGeotabVehicleDetails(cmd *cobra.Command, details geotabVehicleDetails) error {
	out := cmd.OutOrStdout()
