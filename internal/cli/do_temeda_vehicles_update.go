...
type doTemedaVehiclesUpdateOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	ID      string
...
func newDoTemedaVehiclesUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update temeda vehicle assignments",
		Long: `Update temeda vehicle assignments.

Common flags:
  --trailer    Update the trailer assignment
  --tractor    Update the tractor assignment`,
		Example: `  # Update trailer assignment
  xbe do temeda-vehicles update 123 --trailer 456

  # Update tractor assignment
  xbe do temeda-vehicles update 123 --tractor 789`,
		Args: cobra.ExactArgs(1),
		RunE: runDoTemedaVehiclesUpdate,
	}
	initDoTemedaVehiclesUpdateFlags(cmd)
	return cmd
}

func init() {
	doTemedaVehiclesCmd.AddCommand(newDoTemedaVehiclesUpdateCmd())
}

func initDoTemedaVehiclesUpdateFlags(cmd *cobra.Command) {
...
func runDoTemedaVehiclesUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoTemedaVehiclesUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/temeda-vehicles/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated temeda vehicle %s\n", resp.Data.ID)
	return nil
}

func parseDoTemedaVehiclesUpdateOptions(cmd *cobra.Command, args []string) (doTemedaVehiclesUpdateOptions, error) {
	...
	return doTemedaVehiclesUpdateOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		ID:      args[0],
