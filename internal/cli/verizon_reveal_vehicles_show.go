...
type verizonRevealVehiclesShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type verizonRevealVehicleDetails struct {
	ID                    string `json:"id"`
...
func newVerizonRevealVehiclesShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show Verizon Reveal vehicle details",
		Long: `Show the full details of a Verizon Reveal vehicle.
...
Arguments:
  <id>    The Verizon Reveal vehicle ID (required). Use the list command to find IDs.

Global flags (see xbe --help): --json, --base-url, --token, --no-auth`,
		Example: `  # Show a Verizon Reveal vehicle
  xbe view verizon-reveal-vehicles show 123

  # Output as JSON
  xbe view verizon-reveal-vehicles show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runVerizonRevealVehiclesShow,
	}
	initVerizonRevealVehiclesShowFlags(cmd)
	return cmd
}

func init() {
	verizonRevealVehiclesCmd.AddCommand(newVerizonRevealVehiclesShowCmd())
}

func initVerizonRevealVehiclesShowFlags(cmd *cobra.Command) {
...
func runVerizonRevealVehiclesShow(cmd *cobra.Command, args []string) error {
	...
	opts, err := parseVerizonRevealVehiclesShowOptions(cmd)
	if err != nil {
	...
	if id == "" {
		return fmt.Errorf("verizon reveal vehicle id is required")
	}
	...
	body, _, err := client.Get(cmd.Context(), "/v1/verizon-reveal-vehicles/"+id, query)
	if err != nil {
		if len(body) > 0 {
	...
	details := buildVerizonRevealVehicleDetails(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), details)
	}

	return renderVerizonRevealVehicleDetails(cmd, details)
}

func parseVerizonRevealVehiclesShowOptions(cmd *cobra.Command) (verizonRevealVehiclesShowOptions, error) {
	jsonOut, err := cmd.Flags().GetBool("json")
	if err != nil {
		return verizonRevealVehiclesShowOptions{}, err
	}
	...
	return verizonRevealVehiclesShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildVerizonRevealVehicleDetails(resp jsonAPISingleResponse) verizonRevealVehicleDetails {
	attrs := resp.Data.Attributes
	return verizonRevealVehicleDetails{
		ID:                    resp.Data.ID,
...
func renderVerizonRevealVehicleDetails(cmd *cobra.Command, details verizonRevealVehicleDetails) error {
	out := cmd.OutOrStdout()
