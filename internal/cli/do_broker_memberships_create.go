...
type doBrokerMembershipsCreateOptions struct {
	BaseURL                              string
	Token                                string
	JSON                                 bool
	User                                 string
...
func newDoBrokerMembershipsCreateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a broker membership",
		Long: `Create a broker membership.
...
Global flags (see xbe --help): --json, --base-url, --token`,
  ...
  xbe do broker-memberships create --user 123 --broker 456 --title "Manager" --is-admin true

  # Get JSON output
  xbe do broker-memberships create --user 123 --broker 456 --json`,
		Args: cobra.NoArgs,
		RunE: runDoBrokerMembershipsCreate,
	}
	initDoBrokerMembershipsCreateFlags(cmd)
	return cmd
}

func init() {
	doBrokerMembershipsCmd.AddCommand(newDoBrokerMembershipsCreateCmd())
}

func initDoBrokerMembershipsCreateFlags(cmd *cobra.Command) {
...
func runDoBrokerMembershipsCreate(cmd *cobra.Command, _ []string) error {
	opts, err := parseDoBrokerMembershipsCreateOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Post(cmd.Context(), "/v1/broker-memberships", jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Created broker membership %s\n\n", details.ID)
	return renderBrokerMembershipDetails(cmd, details)
}

func parseDoBrokerMembershipsCreateOptions(cmd *cobra.Command) (doBrokerMembershipsCreateOptions, error) {
	...
	return doBrokerMembershipsCreateOptions{
		BaseURL:                              baseURL,
		Token:                                token,
		JSON:                                 jsonOut,
		User:                                 user,
