...
type doProjectPhaseRevenueItemQuantityEstimatesUpdateOptions struct {
	BaseURL            string
	Token              string
	JSON               bool
	ID                 string
...
func newDoProjectPhaseRevenueItemQuantityEstimatesUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update a project phase revenue item quantity estimate",
		Long: `Update a project phase revenue item quantity estimate.
...
Relationships:
  ...
  xbe do project-phase-revenue-item-quantity-estimates update 123 \
    --estimate '{"class_name":"TriangularDistribution","minimum":5,"mode":7,"maximum":12}'

  # Update the description
  xbe do project-phase-revenue-item-quantity-estimates update 123 --description "Updated estimate"

  # Update the estimate set relationship
  xbe do project-phase-revenue-item-quantity-estimates update 123 --project-estimate-set 456`,
		Args: cobra.ExactArgs(1),
		RunE: runDoProjectPhaseRevenueItemQuantityEstimatesUpdate,
	}
	initDoProjectPhaseRevenueItemQuantityEstimatesUpdateFlags(cmd)
	return cmd
}

func init() {
	doProjectPhaseRevenueItemQuantityEstimatesCmd.AddCommand(newDoProjectPhaseRevenueItemQuantityEstimatesUpdateCmd())
}

func initDoProjectPhaseRevenueItemQuantityEstimatesUpdateFlags(cmd *cobra.Command) {
...
func runDoProjectPhaseRevenueItemQuantityEstimatesUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoProjectPhaseRevenueItemQuantityEstimatesUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/project-phase-revenue-item-quantity-estimates/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated project phase revenue item quantity estimate %s\n", row.ID)
	return nil
}

func parseDoProjectPhaseRevenueItemQuantityEstimatesUpdateOptions(cmd *cobra.Command, args []string) (doProjectPhaseRevenueItemQuantityEstimatesUpdateOptions, error) {
	...
	return doProjectPhaseRevenueItemQuantityEstimatesUpdateOptions{
		BaseURL:            baseURL,
		Token:              token,
		JSON:               jsonOut,
		ID:                 args[0],
