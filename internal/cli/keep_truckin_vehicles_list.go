...
type keepTruckinVehiclesListOptions struct {
	BaseURL               string
	Token                 string
	JSON                  bool
	NoAuth                bool
...
type keepTruckinVehicleRow struct {
	ID                    string `json:"id"`
...
func newKeepTruckinVehiclesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List KeepTruckin vehicles",
		Long: `List KeepTruckin vehicles with filtering and pagination.

Output Columns:
  ID         KeepTruckin vehicle identifier
  VEHICLE #  KeepTruckin vehicle number
...
Filters:
  ...
  --trucker-id             Filter by trucker ID (alias)
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
  ...
  # Filter by broker
  xbe view keep-truckin-vehicles list --broker 123

  # Filter by trailer assignment
  xbe view keep-truckin-vehicles list --has-trailer true

  # Filter by active status
  xbe view keep-truckin-vehicles list --active true

  # Output as JSON
  xbe view keep-truckin-vehicles list --json`,
		Args: cobra.NoArgs,
		RunE: runKeepTruckinVehiclesList,
	}
	initKeepTruckinVehiclesListFlags(cmd)
	return cmd
}

func init() {
	keepTruckinVehiclesCmd.AddCommand(newKeepTruckinVehiclesListCmd())
}

func initKeepTruckinVehiclesListFlags(cmd *cobra.Command) {
...
func runKeepTruckinVehiclesList(cmd *cobra.Command, _ []string) error {
	opts, err := parseKeepTruckinVehiclesListOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/keep-truckin-vehicles", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildKeepTruckinVehicleRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderKeepTruckinVehiclesTable(cmd, rows)
}

func parseKeepTruckinVehiclesListOptions(cmd *cobra.Command) (keepTruckinVehiclesListOptions, error) {
	...
	return keepTruckinVehiclesListOptions{
		BaseURL:               baseURL,
		Token:                 token,
		JSON:                  jsonOut,
		NoAuth:                noAuth,
...
func buildKeepTruckinVehicleRows(resp jsonAPIResponse) []keepTruckinVehicleRow {
	included := make(map[string]jsonAPIResource, len(resp.Included))
	for _, inc := range resp.Included {
		included[resourceKey(inc.Type, inc.ID)] = inc
	}

	rows := make([]keepTruckinVehicleRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		rows = append(rows, buildKeepTruckinVehicleRow(resource, included))
	}
	return rows
}

func keepTruckinVehicleRowFromSingle(resp jsonAPISingleResponse) keepTruckinVehicleRow {
	included := make(map[string]jsonAPIResource, len(resp.Included))
	for _, inc := range resp.Included {
		included[resourceKey(inc.Type, inc.ID)] = inc
	}
	return buildKeepTruckinVehicleRow(resp.Data, included)
}

func buildKeepTruckinVehicleRow(resource jsonAPIResource, included map[string]jsonAPIResource) keepTruckinVehicleRow {
	attrs := resource.Attributes
	row := keepTruckinVehicleRow{
		ID:                    resource.ID,
		VehicleID:             stringAttr(attrs, "vehicle-id"),
		VehicleNumber:         stringAttr(attrs, "vehicle-number"),
		CompanyID:             stringAttr(attrs, "company-id"),
		LicensePlateState:     stringAttr(attrs, "license-plate-state"),
		LicensePlateNumber:    stringAttr(attrs, "license-plate-number"),
...
func renderKeepTruckinVehiclesTable(cmd *cobra.Command, rows []keepTruckinVehicleRow) error {
	if len(rows) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No KeepTruckin vehicles found.")
		return nil
	}

	writer := tabwriter.NewWriter(cmd.OutOrStdout(), 2, 4, 2, ' ', 0)
	fmt.Fprintln(writer, "ID\tVEHICLE #\tVEHICLE ID\tACTIVE\tTRACTOR\tTRAILER\tTRUCKER\tBROKER")
