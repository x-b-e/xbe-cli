...
func newNotificationsListCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `List notifications with filtering and pagination.
...
Filters:
  ...
  --deliver-at                 Filter by deliver-at value
  --deliver-at-min             Filter by deliver-at minimum
  --deliver-at-max             Filter by deliver-at maximum
...
Global flags (see xbe --help): --json, --limit, --offset, --sort, --base-url, --token, --no-auth`,
  ...
  # Filter by user
  xbe view notifications list --user 456

  # Filter by read status
  xbe view notifications list --read false
  ...
  xbe view notifications list --json`,
	...
	initNotificationsListFlags(cmd)
	return cmd
}

func init() {
	notificationsCmd.AddCommand(newNotificationsListCmd())
}

func initNotificationsListFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("deliver-at", "", "Filter by deliver-at value")
	cmd.Flags().String("deliver-at-min", "", "Filter by deliver-at minimum")
	cmd.Flags().String("deliver-at-max", "", "Filter by deliver-at maximum")
...
func runNotificationsList(cmd *cobra.Command, _ []string) error {
	opts, err := parseNotificationsListOptions(cmd)
	if err != nil {
	...
	query := url.Values{}
	query.Set("fields[notifications]", "read,deliver-at,is-ready-for-delivery,delivery-decision-approach,notification-type,created-at,updated-at,user")
	...
	body, _, err := client.Get(cmd.Context(), "/v1/notifications", query)
	if err != nil {
		if len(body) > 0 {
	...
	rows := buildNotificationRows(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), rows)
	}

	return renderNotificationsTable(cmd, rows)
}

func parseNotificationsListOptions(cmd *cobra.Command) (notificationsListOptions, error) {
	...
	return notificationsListOptions{
		BaseURL:                  baseURL,
		Token:                    token,
		JSON:                     jsonOut,
		NoAuth:                   noAuth,
...
func buildNotificationRows(resp jsonAPIResponse) []notificationRow {
	included := make(map[string]jsonAPIResource, len(resp.Included))
	for _, inc := range resp.Included {
		included[resourceKey(inc.Type, inc.ID)] = inc
	}

	rows := make([]notificationRow, 0, len(resp.Data))
	for _, resource := range resp.Data {
		rows = append(rows, buildNotificationRow(resource, included))
	}
	return rows
}

func notificationRowFromSingle(resp jsonAPISingleResponse) notificationRow {
	included := make(map[string]jsonAPIResource, len(resp.Included))
	for _, inc := range resp.Included {
		included[resourceKey(inc.Type, inc.ID)] = inc
	}
	return buildNotificationRow(resp.Data, included)
}

func buildNotificationRow(resource jsonAPIResource, included map[string]jsonAPIResource) notificationRow {
...
func renderNotificationsTable(cmd *cobra.Command, rows []notificationRow) error {
	if len(rows) == 0 {
	...
	fmt.Fprintln(writer, "ID\tUSER\tREAD\tREADY\tDELIVER\tAPPROACH\tTYPE")
	for _, row := range rows {
		...
		fmt.Fprintf(writer, "%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
			...
			truncateString(row.NotificationType, 28),
		)
	}
	return writer.Flush()
}
