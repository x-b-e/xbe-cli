...
type doGpsInsightVehiclesUpdateOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	ID      string
...
func newDoGpsInsightVehiclesUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update GPS Insight vehicle assignments",
		Long: `Update GPS Insight vehicle assignments.

Common flags:
  --trailer    Update the trailer assignment
  --tractor    Update the tractor assignment`,
		Example: `  # Update trailer assignment
  xbe do gps-insight-vehicles update 123 --trailer 456

  # Update tractor assignment
  xbe do gps-insight-vehicles update 123 --tractor 789`,
		Args: cobra.ExactArgs(1),
		RunE: runDoGpsInsightVehiclesUpdate,
	}
	initDoGpsInsightVehiclesUpdateFlags(cmd)
	return cmd
}

func init() {
	doGpsInsightVehiclesCmd.AddCommand(newDoGpsInsightVehiclesUpdateCmd())
}

func initDoGpsInsightVehiclesUpdateFlags(cmd *cobra.Command) {
...
func runDoGpsInsightVehiclesUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoGpsInsightVehiclesUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/gps-insight-vehicles/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated GPS Insight vehicle %s\n", resp.Data.ID)
	return nil
}

func parseDoGpsInsightVehiclesUpdateOptions(cmd *cobra.Command, args []string) (doGpsInsightVehiclesUpdateOptions, error) {
	...
	return doGpsInsightVehiclesUpdateOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		ID:      args[0],
