...
type doSamsaraVehiclesUpdateOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	ID      string
...
func newDoSamsaraVehiclesUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <id>",
		Short: "Update samsara vehicle assignments",
		Long: `Update samsara vehicle assignments.

Common flags:
  --trailer    Update the trailer assignment
  --tractor    Update the tractor assignment`,
		Example: `  # Update trailer assignment
  xbe do samsara-vehicles update 123 --trailer 456

  # Update tractor assignment
  xbe do samsara-vehicles update 123 --tractor 789`,
		Args: cobra.ExactArgs(1),
		RunE: runDoSamsaraVehiclesUpdate,
	}
	initDoSamsaraVehiclesUpdateFlags(cmd)
	return cmd
}

func init() {
	doSamsaraVehiclesCmd.AddCommand(newDoSamsaraVehiclesUpdateCmd())
}

func initDoSamsaraVehiclesUpdateFlags(cmd *cobra.Command) {
...
func runDoSamsaraVehiclesUpdate(cmd *cobra.Command, args []string) error {
	opts, err := parseDoSamsaraVehiclesUpdateOptions(cmd, args)
	if err != nil {
	...
	body, _, err := client.Patch(cmd.Context(), "/v1/samsara-vehicles/"+opts.ID, jsonBody)
	if err != nil {
		if len(body) > 0 {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Updated samsara vehicle %s\n", resp.Data.ID)
	return nil
}

func parseDoSamsaraVehiclesUpdateOptions(cmd *cobra.Command, args []string) (doSamsaraVehiclesUpdateOptions, error) {
	...
	return doSamsaraVehiclesUpdateOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		ID:      args[0],
