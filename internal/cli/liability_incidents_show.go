...
type liabilityIncidentsShowOptions struct {
	BaseURL string
	Token   string
	JSON    bool
	NoAuth  bool
...
type liabilityIncidentDetails struct {
	ID                                           string   `json:"id"`
...
func newLiabilityIncidentsShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show <id>",
		Short: "Show liability incident details",
		Long: `Show full details of a liability incident, including key relationships.

Arguments:
  <id>    Liability incident ID (required)

Global flags (see xbe --help): --json, --base-url, --token, --no-auth`,
		Example: `  # Show a liability incident
  xbe view liability-incidents show 123

  # JSON output
  xbe view liability-incidents show 123 --json`,
		Args: cobra.ExactArgs(1),
		RunE: runLiabilityIncidentsShow,
	}
	initLiabilityIncidentsShowFlags(cmd)
	return cmd
}

func init() {
	liabilityIncidentsCmd.AddCommand(newLiabilityIncidentsShowCmd())
}

func initLiabilityIncidentsShowFlags(cmd *cobra.Command) {
...
func runLiabilityIncidentsShow(cmd *cobra.Command, args []string) error {
	opts, err := parseLiabilityIncidentsShowOptions(cmd)
	if err != nil {
	...
	body, _, err := client.Get(cmd.Context(), "/v1/liability-incidents/"+id, query)
	if err != nil {
		if len(body) > 0 {
	...
	details := buildLiabilityIncidentDetails(resp)
	if opts.JSON {
		return writeJSON(cmd.OutOrStdout(), details)
	}

	return renderLiabilityIncidentDetails(cmd, details)
}

func parseLiabilityIncidentsShowOptions(cmd *cobra.Command) (liabilityIncidentsShowOptions, error) {
	jsonOut, _ := cmd.Flags().GetBool("json")
	noAuth, _ := cmd.Flags().GetBool("no-auth")
	baseURL, _ := cmd.Flags().GetString("base-url")
	token, _ := cmd.Flags().GetString("token")

	return liabilityIncidentsShowOptions{
		BaseURL: baseURL,
		Token:   token,
		JSON:    jsonOut,
		NoAuth:  noAuth,
...
func buildLiabilityIncidentDetails(resp jsonAPISingleResponse) liabilityIncidentDetails {
	attrs := resp.Data.Attributes
	details := liabilityIncidentDetails{
		ID:                   resp.Data.ID,
	...
	details.Children = relationshipReferences(resp.Data.Relationships["children"])
	details.IncidentUnitOfMeasureQuantities = relationshipReferences(resp.Data.Relationships["incident-unit-of-measure-quantities"])
	details.RootCauses = relationshipReferences(resp.Data.Relationships["root-causes"])
	details.ActionItems = relationshipReferences(resp.Data.Relationships["action-items"])
	details.IncidentParticipants = relationshipReferences(resp.Data.Relationships["incident-participants"])
	details.IncidentHeadlineSuggestions = relationshipReferences(resp.Data.Relationships["incident-headline-suggestions"])
	details.IncidentTagIncidents = relationshipReferences(resp.Data.Relationships["incident-tag-incidents"])
	details.IncidentTags = relationshipReferences(resp.Data.Relationships["incident-tags"])

	return details
}

func renderLiabilityIncidentDetails(cmd *cobra.Command, details liabilityIncidentDetails) error {
	out := cmd.OutOrStdout()
...
func relationshipReferences(rel jsonAPIRelationship) []string {
	if len(rel.raw) == 0 {
		return nil
	}
